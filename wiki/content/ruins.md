---
title: Ruins & Points of Interest
category: Exploration
order: 4
blurb: Unknown signals, the loot inside them, what is guarding it, and the rare sites you can only reach by buying the rumour.
---

Between the planets and the outposts, the galaxy is full of dead stations, wrecked ships and abandoned facilities. Somewhere between twelve and two dozen of them are scattered across the chart at the start of every round as unknown signals, and more arrive as the round goes on. Boarding one is the fastest way to turn a quiet shift into gear, sheets and a body count.
...
- The value is locked in where the crate spawned. Hauling an unopened cache into a more dangerous zone does not improve it.
- A cache never rolls the same prize twice, so the one-of-a-kind items in the rare tables are genuinely one per crate.

Not every ruin has one. The ones that do keep it in the most defensible room they have.
...
Ten named sites never appear on the chart on their own. The only way to one is to buy the rumour from a trader:
...
Planet surfaces get their own ruins, seeded fresh with the terrain: camps, mining sites, outposts, wrecks and shrines drawn from a pool specific to that world's type. Frozen, volcanic and apocalyptic worlds have the deepest pools; oceanic worlds the shallowest. They use the same caches and the same zone-scaled guards as space ruins, and a handful of them on volcanic, frozen and apocalyptic worlds are purpose-built boss arenas built around a single megafauna. See [Planets](planets.md).
