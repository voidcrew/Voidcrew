...
/datum/unit_test/autopilot_hazard_cache/Run()
	...
	var/obj/structure/overmap/event/nebula/nebula = allocate(/obj/structure/overmap/event/nebula/nitrogen)
	TEST_ASSERT(!(nebula in SSovermap.autopilot_hazards), "Navigable nebula should not be a storm wall")
	ship.build_autopilot_danger_map()
	var/obj/structure/overmap/event/nebula/radioactive = allocate(/obj/structure/overmap/event/nebula/tritium)
	TEST_ASSERT(radioactive in SSovermap.autopilot_hazards, "Radioactive nebula bypassed navigation hazards")
	TEST_ASSERT_NULL(SSovermap.autopilot_blocked_tiles, "Radioactive nebula did not invalidate the cached course")
	ship.build_autopilot_danger_map()
	radioactive.forceMove(get_step(radioactive, NORTH))
	TEST_ASSERT_NULL(SSovermap.autopilot_blocked_tiles, "Moving a radioactive nebula did not invalidate the hazard map")
	ship.build_autopilot_danger_map()
	qdel(radioactive)
	TEST_ASSERT(!(radioactive in SSovermap.autopilot_hazards), "Deleted radioactive nebula remained a hazard")
