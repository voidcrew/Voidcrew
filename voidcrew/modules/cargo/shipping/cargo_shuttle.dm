...
/datum/voidcrew_cargo_shuttle/proc/accept_loan()
	...
	// Announce acceptance
	if(target_ship)
		target_ship.ship_notify(pending_loan.thanks_msg, "CARGO", SHIP_NOTIFY_NOTICE, 'voidcrew/sound/notify.ogg')
...
/datum/voidcrew_cargo_shuttle/proc/complete_arrival()
	...
	if(loan_accepted && pending_loan)
		...
		target_ship?.ship_notify(pending_loan.shuttle_transit_text, "CARGO", SHIP_NOTIFY_NOTICE, 'voidcrew/sound/notify.ogg')
		QDEL_NULL(pending_loan)
		loan_accepted = FALSE
