...
/// Read actual overlays so repainting an existing decal never consumes more pigment.
/// This also works for mapped decals and decals painted by another console.
/obj/machinery/computer/camera_advanced/base_construction/ship/proc/construction_has_decal(turf/target, list/decal, facing)
	...
	// Managed overlays change immediately; BYOND's displayed overlays may still be queued
	// for SSoverlays. Check both so duplicate clicks in one tick cannot spend pigment twice.
	var/list/overlays = target.overlays.Copy()
	if(islist(target.managed_overlays))
		overlays |= target.managed_overlays
	else if(target.managed_overlays)
		overlays |= target.managed_overlays
