/**
 * Remote smoke machine.
 *
 * Ported from monkestation's wiremod_chem module (components/ouputs/smoke_output.dm).
 *
 * Vents everything a circuit pushes into it as chemical smoke the instant it arrives, so
 * the tank never holds a payload between pulses.
 */
/obj/structure/chemical_tank/smoke
	name = "remote smoke machine"
	desc = "A machine with a centrifuge installed into it. It produces smoke with any reagents its given."
	icon = 'icons/obj/medical/chemical.dmi'
	icon_state = "smoke0"
	base_icon_state = "smoke"
	component_name = "Smoke Machine Output"

	reagent_flags = TRANSPARENT

/obj/structure/chemical_tank/smoke/after_reagent_add()
	var/turf/turf = get_turf(src)
	if(isnull(turf))
		return
	if(!reagents.total_volume)
		return

	var/datum/effect_system/fluid_spread/smoke/chem/smoke = new
	smoke.attach(turf)
	smoke.set_up(amount = reagents.total_volume, holder = src, location = turf, carry = reagents, silent = TRUE)
	smoke.start(log = FALSE)

	reagents.clear_reagents()
