...
/datum/reagent/acetone_oxide/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	affected_mob.adjust_organ_loss(ORGAN_SLOT_STOMACH, 2 * REM * seconds_per_tick, required_organ_flag = affected_organ_flags)
	affected_mob.adjust_organ_loss(ORGAN_SLOT_EYES, 2 * REM * seconds_per_tick, required_organ_flag = affected_organ_flags)
