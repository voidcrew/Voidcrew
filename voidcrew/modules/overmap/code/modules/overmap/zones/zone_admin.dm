...
ADMIN_VERB(check_zone_status, R_DEBUG|R_ADMIN, "Zone Status", "Shows current zone system status.", ADMIN_CATEGORY_DEBUG)
	...
	to_chat(user, span_notice("Inner ring (<33%): Red - Dangerous (weapons + interdiction)"))
	to_chat(user, span_notice("Middle ring (33-66%): Yellow - Caution (interdiction only)"))
	to_chat(user, span_notice("Outer ring (>66%): Green - Safe (no PvP)"))
