/obj/structure/overmap/planet
	...
	var/loaded = FALSE
	var/loading = FALSE
	/// Whether a ship has docked at THIS interior since it was built. Decides which
	/// cleanup delay it gets - see get_despawn_delay(). Cleared by unload_level() with
	/// the rest of the interior's state, so a rebuilt planet starts unvisited again.
	var/visited = FALSE
...
/obj/structure/overmap/planet/proc/load_level(mob/user, queue_timeout, obj/structure/overmap/ship/waiting_ship)
	...
	loaded = TRUE
	loading = FALSE
	// Every loaded interior owns a countdown from the moment it exists, whoever built it.
	// A ship arriving cancels it (see Entered()); nothing arriving lets it expire. Before
	// this, on_ship_undocked() was the only thing that ever armed one, so an interior
	// generated by a survey, a transporter lock or a survey-camera refresh - none of which
	// dock anything - stayed resident until roundend. Nine of them were still up on one
	// live round, two of those never docked at even once.
	//
	// Armed BEFORE the signals below, not after: on_site_load_finished() and the survey
	// console's on_survey_planet_loaded() both quote the remaining hold to the crew, and
	// they run inside those signals. Armed afterwards, they would read no countdown at all.
	check_start_despawn()
	SEND_SIGNAL(src, COMSIG_VOIDCREW_PLANET_LOADED, TRUE)
	SEND_SIGNAL(src, COMSIG_VOIDCREW_SITE_LOAD_FINISHED, TRUE)
...
///
/// An interior no ship has ever docked at gets the longer unvisited delay instead: it was
/// charted from orbit by a survey or a transporter lock, and the crew that paid for the
/// chart is usually still deciding whether to land on it.
/obj/structure/overmap/planet/proc/get_despawn_delay()
	var/base_delay = visited ? PLANET_DESPAWN_TIMER : PLANET_UNVISITED_DESPAWN_TIMER
	return max(base_delay, mapzone?.get_ssd_grace_remaining_in(footprint, PLANET_SSD_GRACE_PERIOD))

/// Deciseconds left before the interior is released, or null when nothing is counting
/// down (no armed timer means no interior, a docked ship, or somebody still on the
/// surface - all of which read as "this is not going anywhere"). For readouts.
/obj/structure/overmap/planet/proc/get_interior_hold_remaining()
	if(!despawn_timer_id)
		return null
	// timeleft() answers null for a timer that is no longer there. Every path that spends
	// or cancels the countdown clears despawn_timer_id too, so this should not happen -
	// but "no countdown" is the honest readout for it, not "expires this instant".
	var/remaining = timeleft(despawn_timer_id)
	return isnull(remaining) ? null : max(0, remaining)
...
/obj/structure/overmap/planet/Entered(atom/movable/arrived, atom/old_loc, list/atom/old_locs)
	...
	// Somebody actually came. From here on this interior is on the ordinary abandonment
	// clock rather than the longer one a merely-charted surface gets. Cleared by
	// unload_level() along with the rest of the interior's state.
	visited = TRUE
	cancel_despawn_timer()
