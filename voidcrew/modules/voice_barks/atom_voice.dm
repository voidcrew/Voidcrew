// Core voice/bark datum - ported from Monkestation
// This handles the actual bark sound generation

/datum/atom_voice
	var/datum/voice_pack/voicepack
	var/pitch = 1
	var/pitch_range = 0.2 // Actual pitch is (pitch - (pitch_range*0.5)) to (pitch + (pitch_range*0.5))
	var/base_volume = 300
...
/datum/atom_voice/proc/long_bark(list/hearers, sound_range, volume, is_yell, message_len, atom/movable/speaker)
	...
	var/total_delay = 0
	speaker.long_bark_start_time = world.time // This is just random enough to reliably be unique every time send_speech() is called
