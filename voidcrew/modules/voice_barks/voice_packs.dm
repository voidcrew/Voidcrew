// Voice pack loading system - ported from Monkestation
// Thanks SPLURT, FluffySTG and Citadel
...
/proc/gen_voice_packs()
	if(!fexists(VOICE_PACKS_FILE))
		log_config("No voice packs file found at [VOICE_PACKS_FILE], generating fallback")
	...
	for(var/group_id in output)
		...
		for(var/voice_pack_id in group_obj)
			...
			if(!voice_pack.name)
				stack_trace("Voice pack [voice_pack_id] has no name")
				continue
			...
			if(!voice_pack.sounds[1])
				stack_trace("Voice pack [voice_pack_id] has no talk sound")
				continue
			for(var/i in 2 to 3)
	...
	// Setup goon-only voice_packs system (simple equivalent for non-simple packs)
...
/// Voice pack datum - holds the sound files and parameters for a bark type
/datum/voice_pack
	var/name
	var/id
	var/group_name
