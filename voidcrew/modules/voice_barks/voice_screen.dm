// Voice screen UI for selecting voice packs - ported from Monkestation
// This provides a nice UI with play buttons to preview each voice pack
...
/datum/voice_screen/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	if(!voicepack)
		stack_trace("Failed to locate desired voice pack (path: [params["selected"]]) in the global list of voice packs!")
		return
	...
	switch(action)
		...
		if("play")
			usr.playsound_local(get_turf(usr), voicepack.sounds[1], 300 * voicepack.volume, FALSE, frequency = 1, max_distance = 7, pressure_affected = FALSE, use_reverb = FALSE, channel = CHANNEL_VOICES)
