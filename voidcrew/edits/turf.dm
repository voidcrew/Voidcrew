...
/turf/ChangeTurf(path, list/new_baseturfs, flags)
	...
	if(SSlighting.initialized)
		...
		if(!space_lit)
			// VOIDCREW EDIT: two rules composed here.
			...
			if(wants_lighting_object)
				...
				// Same outcome transfer_area_lighting() reaches via lighting_clear_overlay()
				// when a turf moves into one. Drop a nested duplicate first, since qdel'ing
				// the old object nulls the turf's pointer either way.
