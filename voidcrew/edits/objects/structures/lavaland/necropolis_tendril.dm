...
/obj/structure/spawner/planetary
	name = "corrupted tendril"
...
/obj/structure/spawner/planetary/goliath
	mob_types = list(/mob/living/basic/mining/goliath)
	mob_gps_id = "GL"

/obj/structure/spawner/planetary/legion
	mob_types = list(/mob/living/basic/mining/legion/spawner_made)
	mob_gps_id = "LG"

/obj/structure/spawner/planetary/icewatcher
	mob_types = list(/mob/living/basic/mining/watcher/icewing)
	mob_gps_id = "WT|I" // icewing

GLOBAL_LIST_INIT(planetary_tendrils, list())
/obj/structure/spawner/planetary/Initialize(mapload)
	. = ..()
	emitted_light = new(loc, 4, 2.5, LIGHT_COLOR_LAVA)
	for(var/F in RANGE_TURFS(1, src))
		if(ismineralturf(F))
			var/turf/closed/mineral/M = F
			M.ScrapeAway(null, CHANGETURF_IGNORE_AIR)
	AddComponent(/datum/component/gps, "Eerie Signal")
	GLOB.planetary_tendrils += src

/obj/structure/spawner/planetary/atom_deconstruct(disassembled)
	new /obj/effect/tendril_collapse(loc)

/obj/structure/spawner/planetary/examine(mob/user)
...
/obj/structure/spawner/planetary/Destroy()
	var/last_tendril = TRUE
	if(GLOB.planetary_tendrils.len>1)
		last_tendril = FALSE
...
/obj/effect/tendril_collapse/Initialize(mapload)
	. = ..()
	emitted_light = new(loc, 4, 2.5, LIGHT_COLOR_LAVA)
...
/obj/effect/tendril_collapse/examine(mob/user)
	var/list/examine_messages = ..()
	if(isliving(user))
...
/obj/effect/tendril_collapse/attack_hand(mob/living/collector, list/modifiers)
	. = ..()
...
/obj/effect/tendril_collapse/proc/has_collected(mob/collector)
...
/obj/effect/tendril_collapse/proc/collapse()
