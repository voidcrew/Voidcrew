...
export const BehaviorTreeCompilerTarget = new Juke.Target({
  inputs: [
    '{code,voidcrew}/**/*.bt.json',
    'code/__DEFINES/**/*.dm',
    'tools/build_bt.py',
  ],
  outputs: () => {
    return Juke.glob('{code,voidcrew}/**/*.bt.json').map((file) => {
...
export const BunTarget = new Juke.Target({
  ...
  executes: () => {
    // voidcrew edit - no --frozen-lockfile, the fork's extra workspace package
    // (voidcrew_tgui) drifts from the upstream-maintained bun.lock
    // return bun('./tgui', 'install', '--frozen-lockfile', '--ignore-scripts');
    return bun('./tgui', 'install', '--ignore-scripts');
  },
});
