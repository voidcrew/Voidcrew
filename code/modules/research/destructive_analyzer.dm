...
/obj/machinery/rnd/destructive_analyzer/proc/destroy_item_individual(obj/item/thing, gain_research_points = FALSE)
	...
	if(point_value && !stored_research.deconstructed_items[thing.type])
		...
		stored_research.add_point_list(point_value) // VOIDCREW EDIT - was list(TECHWEB_POINT_TYPE_GENERIC = point_value); point_value is already a point list, so the old call nested a list where add_point_list expects a number and awarded nothing
	qdel(thing)
