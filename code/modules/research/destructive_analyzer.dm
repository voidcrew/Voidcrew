...
/obj/machinery/rnd/destructive_analyzer/proc/destroy_item_individual(obj/item/thing, gain_research_points = FALSE)
	if(isliving(thing))
		...
		mob_thing.death()
	var/list/points = SSresearch.point_items_for(thing) // VOIDCREW EDIT - was techweb_point_items[thing.type]; base-path entries (anomaly cores) need the subtype fallback
	if(length(points) && !stored_research.deconstructed_items[thing.type])
		stored_research.deconstructed_items[thing.type] = TRUE
		stored_research.adjust_multiple_points(points) // VOIDCREW note: upstream's refactor fixed the nested-list zero-award bug our old edit here patched
	qdel(thing)
