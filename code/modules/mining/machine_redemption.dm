...
/obj/machinery/mineral/ore_redemption
	...
	// VOIDCREW EDIT ADDITION START - deposit feedback (rounds 14/15: ore fed to the ORM
	// turned into silo materials and machine-held points with zero on-the-spot feedback,
	// which two crews read as their ore being destroyed)
	...
	/// Cooldown on the warning spoken when ore arrives but the machine cannot take it
	COOLDOWN_DECLARE(cannot_accept_cooldown)
...
/obj/machinery/mineral/ore_redemption/examine(mob/user)
	. = ..()
	// VOIDCREW EDIT ADDITION START - the input tile is not marked on the machine in any way.
	// A crew in round 4 of the 14/15 playtest tipped a satchel out on the wrong side, watched
	// nothing happen, and read it as the machine having eaten their ore.
	. += span_notice("Ore goes in on the [dir2text(input_dir)] side; sheets come out on the [dir2text(output_dir)] side.")
	. += span_notice("Click it with a mining satchel to empty the satchel straight in.")
	// VOIDCREW EDIT ADDITION END
	if(panel_open)
...
/obj/machinery/mineral/ore_redemption/base_item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	...
	for(var/obj/item/stack/ore/gathered_ore as anything in gathered_ores)
		...
		if(smelt_ore(smelted_ore, ID_DATA(user)) <= 0) // VOIDCREW EDIT - every deposit path runs through smelt_ore(), which speaks the summary
...
/obj/machinery/mineral/ore_redemption/pickup_item(datum/source, atom/movable/target, direction)
	...
	if(!materials.mat_container || panel_open || !powered())
		// VOIDCREW EDIT ADDITION START - an unpowered, opened or silo-less machine used to
		// ignore a delivery in total silence, leaving the ore sitting on the input tile with
		// nothing to say why.
		if(istype(target, /obj/item/stack/ore) || istype(target, /obj/structure/ore_box) || istype(target, /obj/item/storage/bag/ore))
			if(COOLDOWN_FINISHED(src, cannot_accept_cooldown))
				COOLDOWN_START(src, cannot_accept_cooldown, 10 SECONDS)
				var/reason = "it is not connected to an ore silo"
				if(!powered())
					reason = "it has no power"
				else if(panel_open)
					reason = "its maintenance panel is open"
				visible_message(span_warning("[src] leaves the ore where it is: [reason]."))
		// VOIDCREW EDIT ADDITION END
		return
	...
	for(var/obj/item/stack/ore/gathered_ore as anything in ore_list)
		...
		if(smelt_ore(smelted_ore, user_data) <= 0) // VOIDCREW EDIT - was a bare materials.insert_item(); smelt_ore() also feeds the spoken deposit summary
	...
	switch(action)
		if("Claim")
			...
			if(requires_silo && !materials.check_z_level())
				say("No ore silo connection. Points cannot be claimed until the machine is linked to a silo.") // VOIDCREW EDIT - silent refusal fix
				return FALSE
			...
			if(isnull(user_id_card))
				...
				return FALSE
			// VOIDCREW EDIT ADDITION START - explain claim refusals; also guards a runtime on account-less IDs
			if(isnull(user_id_card.registered_account))
				say("That ID card has no bank account to hold mining points.")
				return FALSE
			// VOIDCREW EDIT ADDITION END
			...
			if(points)
				...
				say("[points] mining point\s transferred to [user_id_card.registered_account.account_holder]. Mining points are separate from ship credits - spend them at a mining equipment order console.") // VOIDCREW EDIT ADDITION
				points = 0
				return TRUE

			say("No mining points to claim. Points are earned by smelting ore here.") // VOIDCREW EDIT ADDITION
			return FALSE
		if("Release")
			if(!mat_container)
				return
