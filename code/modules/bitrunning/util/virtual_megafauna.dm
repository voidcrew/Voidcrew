...
/mob/living/simple_animal/hostile/megafauna/proc/make_virtual_megafauna()
	...
	// VOIDCREW EDIT: true_spawn gates the GPS component, but that is read in Initialize()
	// and we run well after it, so the component is already attached by the time we get
	// here. Left alone, every megafauna domain broadcasts a lavaland signal out of the
	// turf reservation to anyone holding a GPS.
...
/mob/living/basic/boss/proc/make_virtual_megafauna()
	...
	// VOIDCREW EDIT: same GPS leak as the simple_animal version above, except /basic/boss
	// attaches its beacon with no true_spawn gate at all, so every virtual copy would
	// broadcast out of the turf reservation.
	var/datum/component/gps/beacon = GetComponent(/datum/component/gps)
	if(beacon)
		qdel(beacon)
