...
/datum/unit_test/vestige_field_route/proc/walk_to(turf/destination, avoid_sight = FALSE)
...
/datum/unit_test/vestige_field_route/stillness/Run()
	...
	for(var/exchange in 1 to 3)
		TEST_ASSERT(walk_to(spot(2, 0)), "The student must walk to the two-tile bait position.")
		user.toggle_resting()
		...
		user.toggle_resting()
		TEST_ASSERT(walk_to(spot(1, 1)), "The student must leave both possible sweep lines.")
		sleep(3 SECONDS)
		trial.manual.process(0.2)
...
/datum/unit_test/vestige_field_route/thrown_star/Run()
	...
	for(var/throw_index in 1 to 3)
		...
		TEST_ASSERT(walk_to(firing_positions[throw_index]), "The thrower must physically reach the firing side.")
		...
		TEST_ASSERT(walk_to(center), "The thrower must return to the recall focus.")
...
/datum/unit_test/vestige_field_route/snuffed_flame/Run()
	...
	for(var/index in 1 to 9)
		...
		TEST_ASSERT(walk_to(get_turf(lamp)), "The solver must physically reach each selected lamp.")
...
/datum/unit_test/vestige_field_route/unseen_hand/Run()
	...
	TEST_ASSERT(walk_to(get_turf(southeast_bell)), "The student must walk out of the initial cone to the southeast bell.")
	...
	TEST_ASSERT(walk_to(spot(4, -4), TRUE), "The student must withdraw beyond the patrol's sight.")
	patrol_beats(10)
	TEST_ASSERT(walk_to(get_turf(northwest_bell), TRUE), "The student must approach the other bell outside the sentry's forward cone.")
	...
	TEST_ASSERT(walk_to(spot(2, 3), TRUE), "The student must reach the back of the northeast corner patrol.")
	...
	TEST_ASSERT(walk_to(spot(4, 4), TRUE), "The marked student must first escape the patrol.")
	patrol_beats(13)
	TEST_ASSERT_EQUAL(get_turf(trial.actor), spot(-2, 1), "The sentry must physically patrol to the extraction opening.")
	TEST_ASSERT(walk_to(center, TRUE), "The student must physically reach extraction without entering sight.")
...
/datum/unit_test/vestige_field_route/the_watched/Run()
	...
	TEST_ASSERT(walk_to(get_turf(northwest_lamp), TRUE), "The observer must reach the first lamp behind the watchman.")
	...
	TEST_ASSERT(walk_to(spot(2, 2), TRUE), "The observer must move away before the watchman investigates.")
	patrol_beats(3)
	...
	TEST_ASSERT(walk_to(spot(4, -4), TRUE), "The observer must escape the search for their previous position.")
	...
	TEST_ASSERT(walk_to(get_turf(southeast_lamp), TRUE), "The observer must safely approach a different lamp.")
	...
	TEST_ASSERT(walk_to(spot(-2, 2), TRUE), "The observer must get behind the second repair approach.")
	patrol_beats(5)
	...
	TEST_ASSERT(walk_to(spot(4, 4), TRUE), "The observer must escape the second search.")
	for(var/beat in 1 to 5)
		sleep(1 SECONDS)
		trial.manual.process(1)
	for(var/scan in 1 to 4)
		if(walk_to(center, TRUE))
			break
		patrol_beats(1)
...
/datum/unit_test/vestige_field_route/long_night/Run()
	...
	TEST_ASSERT(walk_to(get_turf(first_refuge)), "The carrier must walk onto the first refuge.")
