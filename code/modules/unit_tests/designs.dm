...
/datum/unit_test/design_source/Run()
	...
	// Voidcrew: designs that are deliberately not reachable from any techweb node or
	// design disk. SSresearch registers every design that has a real ID, so without
	// this list the test flags them as orphans.
