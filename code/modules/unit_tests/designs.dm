...
/datum/unit_test/design_source/Run()
	...
	// VOIDCREW EDIT ADDITION BEGIN - designs that are deliberately not reachable from any
	// techweb node or design disk. SSresearch registers every instantiable design, so
	// without this list the test flags them as orphans.
	all_designs -= list(
