...
/datum/preference_middleware/bark/proc/play_bark(list/params, mob/user)
	...
	if (!barker)
		barker = new()
		barker.voice_bark = new()
	barker.voice_bark.set_from_prefs(preferences)
	barker.voice_bark.long_bark(list(user), 7, 300, FALSE, 32, barker)
...
/datum/preference/numeric/bark_pitch_range/apply_to_human(mob/living/carbon/human/target, value)
	target.voice_bark.pitch_range = value
