...
/obj/machinery/mecha_part_fabricator/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		...
		. += span_notice("Currently configured to drop printed objects <b>[drop_direction ? dir2text(drop_direction) : "onto its own tile"]</b>.") // VOIDCREW EDIT - alt-click reset (voidcrew/modules/research/edits/mech_fabricator.dm) can zero drop_direction
...
/obj/machinery/mecha_part_fabricator/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("remove_mat")
			...
			rmat.eject_sheets(material, amount, user_data = ID_DATA(ui.user)) // VOIDCREW EDIT - was passing usr, so silo logs recorded withdrawals from here as nobody
			return TRUE
