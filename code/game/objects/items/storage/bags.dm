...
/obj/item/storage/bag/ore/proc/pickup_ore(obj/item/ore, mob/user, obj/structure/ore_box/box)
	...
	if (!spam_protection)
		...
		// VOIDCREW EDIT ADDITION START - back the balloon alert with a chat line, so a miner
		// who is watching the ground and not their own sprite finds out that the ore they are
		// walking over is staying on the ground.
