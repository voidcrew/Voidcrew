...
// VOIDCREW EDIT ADDITION BEGIN - upstream hoisted CanBuildHere()/destination_z off
// /turf/open/space and onto /turf/open (code/game/turfs/open/_open.dm), which refuses
// construction on any turf carrying a mirage destination. The fork has always let crews
// lattice/plate space edge turfs even when they double as z-transition tiles, so restore
// that permissive behaviour for space specifically; /turf/open/space/transit still
// overrides this with its own shuttle-bounds check.
/turf/open/space/CanBuildHere()
	return TRUE
// VOIDCREW EDIT ADDITION END
