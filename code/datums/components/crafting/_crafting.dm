...
/datum/component/personal_crafting/proc/get_visible_recipes(mob/user)
	...
	var/list/hidden_schematics
	for(var/datum/crafting_recipe/blueprint/schematic in recipes_to_show)
		if(!is_blueprint_recipe_available(schematic, user))
			LAZYADD(hidden_schematics, schematic)
	if(hidden_schematics)
		recipes_to_show -= hidden_schematics
	//VOID EDIT END
